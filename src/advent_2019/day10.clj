...
(defn parse-asteroid-field [f]
  (let [input (vec (util/lines f))]
...
(defn part1 [f]
  (let [asteroids (parse-asteroid-field f)]
    (->> (map #(count (visible-asteroids % asteroids)) asteroids)
         (apply max))))
...
(defn asteroid-order [station asteroids]
  (let [quarter-circle (/ Math/PI 2)
        full-circle (* 2 Math/PI)
        rotate-angle (fn [angle dangle]
                       (mod (+ angle dangle) full-circle))]
    (->> (visible-asteroids station asteroids)
         ;; Sort all lists in angle order, starting with
         ;; the "up" direction, which means rotating the
         ;; whole circle a quarter
         (sort-by #(rotate-angle (first %) quarter-circle))
         (map second)
         ;; Sort each sub-list of asteroids by distance
         (map #(sort-by (partial dist station) %))
         ;; Finally, take the first from each list, then
         ;; the second, then the third, and so on
         (map #(concat % (repeat nil)))
         (apply map #(remove nil? %&))
         (take-while seq)
         (apply concat))))

(defn part2 [f]
  (let [asteroids (parse-asteroid-field f)
        ;; Find the best station (from part 1)
        station (apply max-key #(count (visible-asteroids % asteroids)) asteroids)
        ;; Sort the asteroids
        sorted-asteroids (asteroid-order station asteroids)
        ;; Find the 200th
        target (->> sorted-asteroids
                    (drop 199)
                    (first))]
    ;; compute the answer
