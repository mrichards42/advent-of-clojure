(ns advent-2019.day07
  ...
  (:require [advent.util :as util]
            [advent-2019.intcode :as ic] ;; the computer
...
(defn amplifiers [code phase-settings]
  (mapv #(-> (ic/program code)
             (ic/send-input [%]))
        phase-settings))

(defn system-1 [code phase-settings]
  (let [amps (amplifiers code phase-settings)
        in (ic/send-input ic/stdin [0 ic/EOF])]
    (ic/system in amps ic/stdout)))

(defn run-system [sys]
  (-> sys
      (ic/run-system)
      (ic/get-program "stdout")
      (ic/read-output)
      (last)))
...
(defn system-2 [code phase-settings]
  (let [amps (amplifiers code phase-settings)
        in (ic/send-input ic/stdin [0 ic/EOF])
        sys (ic/system in amps ic/stdout)
        ;; make the loop
        sys (ic/pipe sys (last amps) (first amps))]
    sys))

(defn part2 [f]
  (let [code (ic/parse-code f)]
    (->> (comb/permutations (range 5 10))
         (map #(run-system (system-2 code %)))
         (apply max))))
