...
(defn claim-squares [{:keys [left top width height]}]
  (for [x (range width)
        y (range height)]
    [(+ left x) (+ top y)]))
...
(defn parse-claim [claim]
  (let [[id left top width height]
        ...
        claim {:id id
               ...
               :width width
               :height height}]
    (assoc claim :squares (set (claim-squares claim)))))

(defn part1 [f]
  (->> (util/lines f)
       (mapcat (comp :squares parse-claim))
       frequencies
       vals
       (filter #(< 1 %))
       count))
...
(defn part2 [f]
  (let [claims (map parse-claim (util/lines f))
        single-claim-squares (->> claims
                                  (mapcat :squares)
                                  frequencies
                                  (keep (fn [[k v]] (when (= 1 v) k)))
                                  set)]
    (->> claims
         (filter (fn [{:keys [squares]}]
                   (every? single-claim-squares squares)))
         first
         :id)))
