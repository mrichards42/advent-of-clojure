...
(def moon-names
  (sort ["Io" "Europa" "Ganymede" "Callisto"]))

(defn parse-moons [f]
  (->> (for [line (util/lines f)]
         (let [pos (->> (re-find #"<x=(\S+), y=(\S+), z=(\S+)>" line)
                        (drop 1)
                        (map #(Integer/parseInt %))
                        (zipmap [:x :y :z]))]
           {:pos pos
            :vel {:x 0 :y 0 :z 0}}))
       (map #(assoc %2 :name %1) moon-names)
       (vec)))

(defn adjust-velocity [moons a b k]
  (let [pa (get-in moons [a :pos k])
        pb (get-in moons [b :pos k])]
    (cond
      (> pa pb) (-> moons
                    (update-in [a :vel k] dec)
                    (update-in [b :vel k] inc))
      (< pa pb) (-> moons
                    (update-in [a :vel k] inc)
                    (update-in [b :vel k] dec))
      :else moons)))

(defn adjust-velocities [moons]
  (reduce (fn [moons [a b]]
            (-> moons
                (adjust-velocity a b :x)
                (adjust-velocity a b :y)
                (adjust-velocity a b :z)))
          moons
          ;; optimization: manually calculate the combinations
          (list [0 1] [0 2] [0 3]
                [1 2] [1 3]
                [2 3])))
...
(defn move-moon [moon]
  (reduce (fn [moon k]
            (update-in moon [:pos k] + (get-in moon [:vel k])))
          moon
          [:x :y :z]))

(defn move-moons [moons]
  (mapv move-moon moons))
...
(defn energy [moon]
  (let [potential (reduce + (map #(Math/abs (get-in moon [:pos %])) [:x :y :z]))
        kinetic (reduce + (map #(Math/abs (get-in moon [:vel %])) [:x :y :z]))]
    (* potential kinetic)))
...
(defn axis-period
  "Count the number of steps until the position and velocity of `axis` repeat."
  [simulation axis]
  (let [;; extract just the position and velocity for `k`
        axis-simulation (for [moons simulation]
                          (for [moon moons]
                            (mapv axis (map moon [:pos :vel]))))
        first-state (first axis-simulation)]
    (->> (drop 1 axis-simulation)
         (take-while #(not= first-state %))
         (count)
         ;; b/c we dropped 1
         (inc))))

(defn system-period [moons]
  (let [simulation (iterate step moons)
        ;; Each axis moves independent of the others, so we need to find the
        ;; LCM for the period of each axis
        x (axis-period simulation :x)
        y (axis-period simulation :y)
        z (axis-period simulation :z)]
    (reduce math/lcm [x y z])))

(defn part2 [f]
  (system-period (parse-moons f)))
