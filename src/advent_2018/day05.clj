...
(defn reaction?
  [^Character a ^Character b] ;; this is the hot path
  (and (not= a b)
       (= (Character/toLowerCase a)
          (Character/toLowerCase b))))

(defn reduce-polymers [input]
  (letfn [(step [result input]
            (if (and (seq result) (reaction? (peek result) input))
              (pop result)
              (conj result input)))]
    (reduce step [] input)))
...
(defn part2 [f]
  (let [input (apply str (reduce-polymers (util/slurp f)))]
    (apply
     min
     (for [c (char-range \a \z)]
       (let [pattern (re-pattern (str "[" c (Character/toUpperCase c) "]"))
             input (str/replace input pattern "")]
         (count (reduce-polymers input)))))))
