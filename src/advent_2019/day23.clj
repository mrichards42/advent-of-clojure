...
(defn read-packets [network]
  (reduce (fn [packets [addr X Y]]
            (update packets addr (fnil conj []) X Y))
          {}
          (partition-all 3 (mapcat ic/read-output (:programs network)))))

(defn deliver-packets [network packets]
  (-> network
      (update :programs (fn [programs]
                          (map-indexed (fn [idx program]
                                         (-> program
                                             (ic/consume-output)
                                             (ic/send-input (get packets idx [-1]))))
                                       programs)))
      (update :NAT #(or (take-last 2 (get packets 255)) %))))

(defn run-network-step [network]
  (let [network (update network :programs #(map ic/resume %))
        packets (read-packets network)]
    (if (or (seq packets) (empty? (:NAT network)))
      ;; send packets as normal
      (deliver-packets network packets)
      ;; send the NAT packet to 0
      (-> network
          (update :sent-by-nat conj (:NAT network))
          (deliver-packets {0 (:NAT network)})))))

(defn part1 [f]
  (let [program (ic/program (ic/parse-code f))
        network (init-network program 50)]
    (->> (iterate run-network-step network)
         (drop-while (complement :NAT))
         (first)
         (:NAT)
         (second) ;; Y
         )))
...
(defn part2 [f]
  (let [program (ic/program (ic/parse-code f))
        network (init-network program 50)]
    (->> (iterate run-network-step network)
         (drop-while #(= (count (set (:sent-by-nat %)))
                         (count (:sent-by-nat %))))
         (first)
         (:sent-by-nat)
         (last)
