...
(defn part1 [f]
  (let [counts (map letter-counts (util/lines f))
        twos (count (filter #(contains? % 2) counts))
        threes (count (filter #(contains? % 3) counts))]
    (* twos threes)))
...
(defn similar-letters [a b]
  (->> (map #(when (= %1 %2) %2) a b)
       (filter identity)))

(defn similar? [a b]
  (<= (- (count a) (count (similar-letters a b))) 1))

(defn similar-match [[item & rest-inputs]]
  (if-let [match (first (filter (partial similar? item) rest-inputs))]
    [item match]
    (recur rest-inputs)))

(defn part2 [f]
  (let [[a b] (similar-match (util/lines f))]
    (apply str (similar-letters a b))))
