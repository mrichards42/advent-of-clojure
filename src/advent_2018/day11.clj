...
(defn grid-power-level-naive [serial-number x y size-x size-y]
  (->> (for [dx (range 0 size-x)
             dy (range 0 size-y)]
         (power-level serial-number (+ x dx) (+ y dy)))
       (reduce +)))

(defn part1 [f]
  (let [serial-number (Integer/parseInt (util/slurp f))]
    (->> (for [x (range 1 299)
               y (range 1 299)]
           {:top-left [x y]
            :power-level (grid-power-level-naive serial-number x y 3 3)})
         (apply max-key :power-level)
         (:top-left))))
...
;; I needed a hint for this: use a summed-area table

(defn all-power-levels [serial-number size]
  (vec (for [y (range 1 (inc size))]
         (vec (for [x (range 1 (inc size))]
                (power-level serial-number x y))))))

(defn summed-area-table [table]
  (->> table
       ;; Compute the running total within each row
       (map (comp vec (partial reductions +)))
       ;; Compute the running total across rows
       (reductions (partial mapv +))
       (vec)))

(defn table-get [table x y]
  (-> table
      ;; we're using 1-indexed cells
      (get (dec y))
      (get (dec x))))

(defn grid-power-level [table x y size-x size-y]
  (let [x (+ x (dec size-x))
        y (+ y (dec size-y))
        bottom-right (or (table-get table x y) 0)
        top-left (or (table-get table (- x size-x) (- y size-y)) 0)
        bottom-left (or (table-get table (- x size-x) y) 0)
        top-right (or (table-get table x (- y size-y)) 0)]
    (+ bottom-right
       top-left
       (- bottom-left)
       (- top-right))))
...
(defn part2 [f]
  (let [serial-number (Integer/parseInt (util/slurp f))
        table (summed-area-table (all-power-levels serial-number 300))]
    (->> (for [size (range 1 301)
               x (range 1 (- 301 size))
               y (range 1 (- 301 size))]
           {:top-left [x y size]
            :power-level (grid-power-level table x y size size)})
         (apply max-key :power-level)
         (:top-left))))
