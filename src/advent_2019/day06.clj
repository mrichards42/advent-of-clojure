...
(defn parse-orbit-map [f]
  (->> (util/lines f)
       (map #(str/split % #"\)"))
       (reduce (fn [graph [a b]]
                 (-> graph
                     (update-in [a :children] (fnil conj #{}) b)
                     (assoc-in [b :parent] a)))
               {})))

(defn planet-ranks [g]
  (iterate (fn [planets] (mapcat #(get-in g [% :children]) planets))
           ["COM"]))

(defn orbit-count [g]
  (->> (planet-ranks g)
       (take-while seq)
       (map-indexed (fn [idx planets] (* idx (count planets))))
       (reduce +)))

(defn part1 [f]
  (let [g (parse-orbit-map f)]
    (orbit-count g)))
...
(defn path-to-center [g planet]
  (->> (iterate #(get-in g [% :parent]) planet)
       (take-while identity)))

(defn transfers-between [g a b]
  (let [a-path (path-to-center g a)
        b-path (path-to-center g b)
        ; a-path '[YOU K J E D C B COM]
        ; b-path '[SAN I D C B COM]
        common-path (->> (map vector (reverse a-path) (reverse b-path))
                         (take-while #(apply = %))
                         (map first))]
    (+ (count a-path)
       (count b-path)
       (- (* 2 (count common-path)))
       ;; the last 2 edges don't count since they already represent a planet
       ;; orbiting around another planet
       (- 2))))

(defn part2 [f]
  (let [g (parse-orbit-map f)]
    (transfers-between g "YOU" "SAN")))
