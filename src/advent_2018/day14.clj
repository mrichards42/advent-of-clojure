...
(defn create-recipes [recipes ^Integer a ^Integer b]
  (let [total (+ (get recipes a) (get recipes b))]
    (if (< total 10)
      (conj recipes total)
      (-> recipes
          (conj (quot total 10))
          (conj (rem total 10))))
    ;; optimization: into is a good deal slower than 2 conj's
    #_(into recipes (if (< total 10)
                      [total]
                      [(quot total 10) (rem total 10)]))))

(defn select-new-recipe [recipes ^Integer idx]
  (rem (+ (int idx) (inc (int (get recipes idx))))
       (count recipes)))

(defn recipe-seq [recipes a b]
  (let [recipes' (create-recipes recipes a b)
        a' (select-new-recipe recipes' a)
        b' (select-new-recipe recipes' b)]
    (lazy-seq (cons recipes' (recipe-seq recipes' a' b')))))

(defn part1 [f]
  (let [target (Integer/parseInt (util/slurp f))]
    (->> (recipe-seq [3 7] 0 1)
         (drop-while #(< (count %) (+ 10 target)))
         (first)
         (drop target)
         (take 10)
         (apply str))))
...
(defn ends-with? [recipes target]
  (= target (subvec recipes (- (count recipes) (count target)) (count recipes))))

(defn ends-with?-1 [recipes target]
  (= target (subvec recipes (- (count recipes) (count target) 1) (- (count recipes) 1))))

(defn has-target? [recipes target]
  (or
   (ends-with? recipes target)
   (ends-with?-1 recipes target)))

(defn part2 [f]
  (let [target (mapv #(Integer/parseInt (str %)) #_"59414" (util/slurp f))
        recipes (->> (recipe-seq [3 7] 0 1)
                     (drop-while #(< (count %) (inc (count target))))
                     (drop-while #(not (has-target? % target)))
                     (first))]
    (if (ends-with? recipes target)
      (- (count recipes) (count target))
      (- (count recipes) (count target) 1))))
