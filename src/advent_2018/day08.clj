...
(defn parse-node
  ([data] (parse-node data 0))
  ([data i]
   (let [child-count (get data i)
         meta-count (get data (inc i))
         header-end (+ 1 i)
         children (->> (iterate #(parse-node data (inc (:end %)))
                                {:end header-end})
                       (drop 1)
                       (take child-count)
                       vec)
         children-end (or (:end (last children)) header-end)
         meta-end (+ children-end meta-count)]
     {:children children
      :metadata (vec (subvec data (inc children-end) (inc meta-end)))
      :start i
      :end meta-end})))

(defn parse-license [f]
  (->> (str/split (util/slurp f) #" ")
       (mapv #(Integer/parseInt %))
       (parse-node)))

(defn all-meta [{:keys [children metadata]}]
  (apply concat metadata
         (map all-meta children)))

(defn part1 [f]
  (reduce + (all-meta (parse-license f))))
...
(defn node-val [{:keys [children metadata]}]
  (if (seq children)
    (->> (keep #(get children (dec %)) metadata)
         (map node-val)
         (reduce +))
    (reduce + metadata)))

(defn part2 [f]
  (node-val (parse-license f)))
