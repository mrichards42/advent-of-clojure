...
(defn parse-layers [f w h]
  (->> (util/slurp f)
       (partition-all (* w h))
       (map (fn [layer] (mapv #(Integer/parseInt (str %)) layer)))
       vec))
...
(def image (parse-layers (util/data) 25 6))
...
(defn decode [layers]
  (let [pixels (apply map vector layers)]
    (for [pixel-layers pixels]
      (->> pixel-layers
           (drop-while #{2})
           (first)))))

(defn image-data [pixels w h]
  (->> (map #(if (= 1 %) "█" ".") pixels)
       (partition-all w)
       (map str/join)
       (str/join "\n")))
