...
(defn parse-program [f]
  (mapv #(Integer/parseInt %) (str/split (util/slurp f) #",")))

(defn run-program*
  [data ip]
  (let [[op a b c] (subvec data ip)
        a (get data a)
        b (get data b)
        ip' (+ 4 ip)]
    (case op
      1 (recur (assoc data c (+ a b)) ip')
      2 (recur (assoc data c (* a b)) ip')
      99 data
      (throw (ex-info (str "Unexpected op:" op)
                      {:data data
                       :ip ip
                       :op op})))))

(defn run-program [data noun verb]
  (-> (assoc data 1 noun 2 verb)
      (run-program* 0)
      (first)))

(defn part1 [f]
  (run-program (parse-program f) 12 2))
...
(defn noun-verb-search [data target]
  (->> (for [noun (range 100)
             verb (range 100)]
         [noun verb])
       (filter #(= target (apply run-program data %)))
       (first)))

(defn part2 [f]
  (let [data (parse-program f)
        target 19690720
        [noun verb] (noun-verb-search data target)]
    (+ (* 100 noun) verb)))
